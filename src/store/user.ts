...
export const authStore = defineStore({
  ...
  getters: {
    ...
    getDepositCfg: (state) => state.depositcfg,
  ...
  actions: {
    ...
    async dispatchUserDepositCfg() {
      this.setSuccess(false);
      const route: string = NETWORK.PERSONAL_INFO_PAGE.USER_DEPOSIT_CONFIG;
      const network: Network = Network.getInstance();
      // response call back function
      const next = (response: User.GetUserInfoResponseData) => {
        if (response.code == 200) {
    ...
    // user deposit configuration
    async dispatchUserDepositSubmit() {
      this.setSuccess(false);
      const route: string = NETWORK.PERSONAL_INFO_PAGE.USER_DEPOSIT_SUBMIT;
      const network: Network = Network.getInstance();
      // response call back function
      const next = (response: User.GetUserInfoResponseData) => {
        if (response.code == 200) {
