...
const Login = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const hiddenGoogleBtn = () => {
      googleBtn.value.style.display = 'none'
    }
    /**
     * 初始化google SDK
     */
    const initGoogle = async () => {
      ...
      google.account.id.initialize({
        client_id: 'appId' || 'none',
        callback: (data: { credential: string }) => {
          this.loginCallBack(ThirdWayType.GOOGLE, {
            'access_token': 'token',
            'email': ''
          })
          hiddenGoogleBtn()
      ...
      const lang = 'en'
      google.accounts.id.renderButton(googleBtn.vale, {
        type: 'standard',
        theme: 'filled_blue',
        text: 'continue_with',
        locale: lang
    ...
    const hiddenGoogleBtn = () => {
      document.getElementById('google-btn-help').style.display = 'none'
      document.removeEventListener('click', hiddenGoogleEvent)
    };
    const hiddenGoogleEvent = (ev) => {
      if (document.getElementById('google-btn-help').contains(ev.target)) {
        hiddenGoogleBtn()
      }
    };
    const showGoogleBtn = () => {
      if (document.getElementById('google-btn-help').style.display === 'block') {
        document.removeEventListener('click', hiddenGoogleBtn)
      }
      document.getElementById('google-btn-help').style.display = 'block'
      document.addEventListener('click', hiddenGoogleEvent)
    }
    const loginByGoogle = (end?: () => void) => {
      return new Promise(async (resolve, reject) => {
        let loginGoogleResolve = resolve
        let winInitialize = typeof window?.google?.accounts?.id?.initialize === 'function'
        if (!winInitialize) {
          initGoogle()
          await delay(2000)
        } else {
        ...
        showGoogleBtn()
        const googleIframe = document.querySelector('#google-btn-help iframe')
        if (googleIframe?.style?.width === '0px') {
          let srcVal = googleIframe.src
          googleIframe.src = srcVal
          await delay(2000)
          if (googleIframe?.style?.width === '0px') {
            return reject('Google.login error')
          }
        }
      })
    }
    ...
    const loginByFacebook = (end?: () => void) => {
          ...
          function (response) {
            if (response.status === 'connected') {
              ...
              FB?.api(
                ...
                function (data: { email?: string; id?: string }) {
                  if (data) {
                    resolve({
    ...
    /**
     * 三方綁定和登錄（google, facebook）
     * @param type
     * @param mode
     * @param beforeLoading
     * @param afterLoading
     */
    const threeWayBindOrLogin = (type, mode, beforeLoading?: () => void, afterLoading?: () => void) => {
      // 是否登錄
      const hasLogin = 0;
      if (hasLogin) {
        router.replace({
        ...
        if (mode === 'login') {
          try {
            ...
            switch (type) {
              case 'google':
                loginInfos = await loginByGoogle(afterLoading)
                break
              case 'facebook':
                loginInfos = await loginByFacebook(afterLoading)
                break
              default:
                break
            }
            beforeLoading?.()
            const requester_type = type
            const response = '登錄彈窗信息'
            const loginModal = document.querySelector('.loginModal')
            if (loginModal) {
              console.log('關閉登錄彈窗')
            }
            if (response) {
              const email = loginInfos?.email || response.email
              console.log('注冊彈窗')
            }
          } catch (error) {
            console.log(error)
