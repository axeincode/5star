...
<template>
    <v-row class="mx-2 mt-0">
        <div class="relative m-agent-report-date-picker" @click="datePickerShow = true">
            <el-date-picker v-model="selectedDate" popper-class="m-agent-report-date-picker-background" type="daterange"
                value-format="YYYY/MM/DD" :format="dateFormat" :prefix-icon="customPrefix" :visible="datePickerShow"
                :clear-icon="customClear" start-placeholder="Start date" end-placeholder="End date"
                :popper-options="popperOptions" day-name-format="short" :start-day-of-week="1"
                @change="datePickerShow = false" @blur="datePickerShow = false" size="small">
    ...
    <v-row class="mt-6 mx-3">
        <v-menu offset="10" content-class="m-agent-report-bonus-menu" v-model="bonusMenuShow"
            ...
            <v-list theme="dark" class="m-agent-report-bonus-menu-list">
                <v-list-item v-for="(item, i) in inviteHistoryConfig.list" :key="i" :value="item.name"
                    class="bonus-item mx-2" @click="handleHistoryConfigDropdown(item)"
                    :class="selectedBonusItem == item.name ? 'm-agent-report-bonus-menu-selected-item' : ''">
        ...
        <v-menu offset="10" content-class="m-agent-report-bonus-menu" v-model="cashMenuShow"
            ...
            <v-list theme="dark" class="m-agent-report-bonus-menu-list">
                <v-list-item v-for="(item, i) in cashItems" :key="i" :value="item" class="bonus-item mx-1"
                    :class="selectedPageSize == item ? 'm-agent-report-bonus-menu-selected-item' : ''"
    ...
    <v-row class="mx-2 mt-6 m-agent-forms-bonus-table">
        <v-table class="m-agent-forms-bonus-table-bg" :class="fixPositionShow ? 'table-position-overflow' : ''" theme="dark"
            fixed-header>
            <thead class="forms-table-header">
                <tr>
                    <th class="m-forms-table-header-text px-0" style="border-radius: 8px 0px 0px 8px">
                    ...
                    <th class="m-forms-table-header-text px-0" style="border-radius: 0px 8px 8px 0px">
                        {{ t("affiliate.forms.table.bonus") }}
                    </th>
                </tr>
            </thead>
            <tbody class="m-forms-table-body">
                <tr v-for="(item, index) in inviteHistoryItem.list" :key="index" v-if="inviteHistoryItem.list.length > 0">
