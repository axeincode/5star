        ...
        <v-table class="forms-bonus-table-bg" theme="dark" fixed-header height="700px">
            ...
            <tbody class="forms-table-body">
                <tr v-for="(item, index) in formsList" :key="index">
                    ...
                    <td class="text-400-14" :class="item.status == 'In Processing' ? 'color-F9BC01' : ''">{{ item.status }}
                    </td>
                    <td class="text-400-14" :class="item.status == 'In Processing' ? 'color-F9BC01' : ''">{{ item.note }}
                    </td>
