...
const emit = defineEmits<{
  ...
  (e: "submitEmail", name: string): void;
}>();
...
const isFormDataReady = computed(
  (): boolean => validateEmail() && passwordValidationList.value
);
  ...
  <div class="m-user-container">
    ...
    <v-row class="mt-6 relative" :class="mobileWidth < 600 ? 'ma-2' : 'ma-10'">
      <v-text-field
        ...
        class="form-textfield dark-textfield m-account-email-edit-address"
        variant="solo"
        density="comfortable"
        v-model="email"
      />
    ...
    <v-row class="relative" :class="mobileWidth < 600 ? 'ma-2' : 'ma-10'">
      <v-text-field
        :label="t('account.password.current_text')"
        class="form-textfield dark-textfield m-account-email-edit-password"
        variant="solo"
        density="comfortable"
