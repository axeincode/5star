...
const emit = defineEmits<{
  ...
  (e: "submitNickName", name: string): void;
}>();
...
const nickNameValidationList = computed((): boolean[] => {
  ...
  const condition2 = !(
    nickName.value.toLowerCase().trim() === email.value.toLowerCase().trim()
  );
...
const validateNickName = (): boolean => {
  return (
    nickNameValidationList.value.reduce((res, item) => res && item, true) &&
    nickName.value != "DanDan"
  );
};
...
<template>
  <div class="m-user-container">
    ...
    <v-row class="relative ma-2" style="margin-top: 110px !important">
      <v-text-field
        :label="t('account.item.nick_name_text')"
        class="form-textfield dark-textfield m-account-nickname-edit"
        variant="solo"
        density="comfortable"
        v-model="nickName"
      ...
      <ValidationBox
        ...
        :validationList="nickNameValidationList"
      />
      <ExistValidationBox
        v-if="isExistValidation"
        :title="t('account.exist_validation_text')"
        :withCautionIcon="true"
      />
    </v-row>
    <v-row style="margin-top: 70px !important" class="ma-2">
      <v-btn
        class="ma-3 mt-8 button-bright m-account-dialog-save-btn"
    ...
    <v-btn
      class="m-account-dialog-close-btn"
      icon="true"
