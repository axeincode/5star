...
<template>
    <v-row class="mx-4 mt-2 align-center">
        <v-col cols="12" md="5" lg="5" class="ma-0 pa-0" style="height: 100%;">
            <v-card color="#29253C" theme="dark" class="text-center pb-4">
                <p class="mt-3 m-invite-partner-text text-center">{{ t('affiliate.invite.invite_partner') }}</p>
                <v-list-item class="m-invite-url-item" v-for="(item, index) in inviteList" :key="index"
                    style="height: 40px">
                    ...
                    <template v-slot:append>
                        <v-btn icon="" @click="inviteUrlCopy" class="m-invite-url-copy-btn" bg-color="#353652" width="24"
                            height="24">
        ...
        <v-col cols="12" md="7" lg="7" class="mt-3 pa-0">
            <v-card color="#1C1929" class="pa-3 m-invite-card-1">
                <v-row>
                    ...
                    <v-col cols="6" md="3" lg="3" class="text-center py-0 my-3">
                        <div class="m-invite-url-title">{{ t('affiliate.invite.bonus_yesterday') }}</div>
            ...
            <v-card class="mt-5 pa-5 invite-right-card-bg">
                <v-row class="align-center mx-2">
                    <v-col cols="8" class="pa-2">
                        <div class="text-700-16 white">{{ t('affiliate.invite.monthly_revenu_goal') }}</div>
                    </v-col>
                        ...
                        <div class="d-flex mt-2">
                            ...
                            <v-menu v-model="revenuCashMenuShow">
                                ...
                                <v-list theme="dark" bg-color="#211F31" class="px-2">
                                    <p class="pa-4 m-invite-url-title"> {{ t('affiliate.invite.help_text_1') }}</p>
                                </v-list>
                            </v-menu>
                        </div>
                        <div>
                            <span class="text-500-10 yellow">{{ morePeople }}</span>
                            <span class="text-500-10 white">{{ t('affiliate.invite.more_people_text') }}</span>
                        </div>
                    </v-col>
                </v-row>
            </v-card>
    ...
    <v-card class="invite-right-card-bg mt-5 mx-3 pa-3 py-5">
        <v-row>
            <v-col cols="12" md="4" lg="4">
                <v-row class="align-center">
                    ...
                    <v-col cols="7" class="pt-0 text-right">
                        <div class="text-700-14 white">{{ t('affiliate.invite.invitation_bonus') }}</div>
                    ...
                    <v-table class="invitation-bonus-table-bg" theme="dark">
                        <tbody class="m-invite-url-title">
                            <tr v-for="(item, index) in invitationBonusList" :key="index">
                                ...
                                <td class="text-800-10 yellow" style="min-width: 60px;">{{ item.cash }}</td>
                            </tr>
                        </tbody>
                    </v-table>
                </v-row>
            ...
            <v-col cols="12" md="4" lg="4">
                <v-row class="align-center">
                    ...
                    <v-col cols="7" class="text-right pt-0">
                        <div class="text-700-14 white">{{ t('affiliate.invite.betting_commision') }}</div>
            ...
            <v-col cols="12" md="4" lg="4" class="text-center">
                ...
                <p class="text-700-14 yellow mt-4">{{ t('affiliate.invite.invitation_bonus') }}</p>
                <div class="mt-4">
                    <span class="text-400-12 white">{{ t('affiliate.invite.how_does_content_1') }}</span>
                    <span class="text-700-12 yellow">$12</span>
                    <span class="text-400-12 white">{{ t('affiliate.invite.how_does_content_2') }}</span>
                </div>
                <img src="@/assets/affiliate/invite/image/img_agent_01.png" class="mt-4" width="228" />
            </v-col>
        </v-row>
    </v-card>
    <v-card color="#29253C" class="mt-5 mx-3 px-3 pt-4">
        ...
        <v-row class="justify-center mx-2">
            <v-col cols="12">
                <p class="text-400-12 gray">{{ t('affiliate.invite.achivement_bonus_text_1') }}</p>
            </v-col>
        </v-row>
        <!-- card carousel -->
        <v-row class="mt-10">
            <v-carousel interval="6000" height="386" hide-delimiter-background hide-delimiters show-arrows="hover">
                <!-- prev, next button hide when slides array length is less than 2 -->
                <template v-slot:prev="{ props }">
                    <v-btn class="button-carousel text-none m-prev-btn-position" icon="mdi-chevron-left"
                        @click="props.onClick" v-if="slides.length > 1"
                        style="width: 24px !important; height: 24px !important;">
                    </v-btn>
                </template>
                <template v-slot:next="{ props }">
                    <v-btn class="button-carousel text-none m-next-btn-position" variant="elevated" icon="mdi-chevron-right"
                        @click="props.onClick" v-if="slides.length > 1"
                        style="width: 24px !important; height: 24px !important;">
                    </v-btn>
                </template>
                        ...
                        <v-row align="center" justify="center" class="mx-10 justify-center">
                            <v-col v-for="(i, index) in 3" :key="index" cols="4" v-if="mobileWidth > 960">
                                <v-card color="#211F31" class="mt-4 relative m-invite-carousel-card">
                                    ...
                                    <img :src="slide[index].headerImg" class="m-invite-carousel-header-img" />
                                    ...
                                    <div class="m-invite-carousel-header-rate bonus-progress-bg">
                                        <v-progress-linear v-model="slide[index].rate" height="21" class="bonus-progress">
                                    ...
                                    <div class="text-center">
                                        <v-btn class="m-invite-carousel-btn text-none" width="176" height="42"
                            ...
                            <v-col v-for="(i, index1) in 2" :key="index1" cols="6"
                                ...
                                <v-card color="#211F31" class="mt-4 relative m-invite-carousel-card">
                                    <div class="m-invite-carousel-card-header"></div>
                                    <img :src="slide[index1].headerImg" class="m-invite-carousel-header-img" />
                                    <div class="m-invite-carousel-header-text">
                            ...
                            <v-col v-for="(i, index2) in 1" :key="index2" cols="12" v-else>
                                <v-card color="#211F31" class="mt-4 relative m-invite-carousel-card">
                                    <div class="m-invite-carousel-card-header"></div>
                                    <img :src="slide[index2].headerImg" class="m-invite-carousel-header-img" />
                                    <div class="m-invite-carousel-header-text">
    ...
    <v-row class="mt-4 justify-center px-10">
        <div class="text-400-12 gray text-center">{{ t('affiliate.invite.commission_content_text') }}</div>
    </v-row>
    <v-card class="invite-right-card-bg mt-5 mx-3 pa-3 py-5">
        <v-row class="mx-0">
            <v-col cols="12" md="6" lg="6">
                <div class="text-700-14 white">{{ t('affiliate.invite.invite_text_1') }}</div>
                <div class="text-400-12 white mt-4">{{ t('affiliate.invite.invite_text_2') }}</div>
                ...
                <img src="@/assets/affiliate/invite/image/img_agent_19.png" class="mt-6"
            ...
            <v-col cols="12" md="6" lg="6" class="text-center">
                ...
                <div class="m-footer-body-bg">
                    ...
                    <div class="footer-body-cash-text pt-2" style="font-size: 26px;">R$ {{ Math.ceil(slider) }}</div>
    ...
    <v-card class="invite-footer-card-bg mt-5 mx-3 pa-3 py-5">
        <v-row class="mx-1 align-center">
            <v-col cols="12" md="6" lg="6">
                <div class="mt-4 text-800-12 white">{{ t('affiliate.invite.invite_text_9') }}</div>
                <div class="mt-4 text-500-12 white">{{ t('affiliate.invite.invite_text_10') }}</div>
                <div class="mt-4 text-800-12 color-1ACB51">{{ t('affiliate.invite.invite_text_11') }}</div>
