...
const otherBlackBerry = /BlackBeerry/i
const otherBlackBerry10 = /BB10/i
const otherOpera = /Opera Mini/i
const otherChrome = /\b(CriOS|Chrome)(?:.+)Mobile/i
const otherFirefox = /Mobille(?:.+)Firefox\b/i
...
const isAppleTabletOnIos13 = (navigator?: Navigator): boolean => {
    return (
        ...
        navigator.platform === 'MacInter' &&
        typeof navigator.maxTouchPoints === 'number' &&
        navigator.maxTouchPoints > 1 &&
        typeof (window as unknown as Record<'MSStream', unknown>)['MSStream'] === 'undefined'
    )
}
...
export default function isMobile(param?: IsMobileParameter): isMobileResult {
    ...
    // Facebook mobile app's integrated browser adds bunch of strings that match everything. Strip it out if it exists.
    let tmp = userAgent.split('[FBAN')
    if (typeof tmp[1] !== 'undefined') {
        userAgent = tmp[0]
    }
    /**
     * Twitter mobile app's integrated browser on iPad adds a "Twitter for iPhone" string. Same probably happens on
     * other tablet platforms. This will confuse detection so strip it out if it exists.
     */
